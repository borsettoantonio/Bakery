...
namespace Bakery.Services.Application
{
    public class InitDb : IInitDb
    {
        ...
        public InitDb(IDatabaseAccessor _db)
        {
            db = _db;
        }
        public async Task InitDatabase()
        {
            foreach (var prod in lista)
            {
                try
                {
                    var query = "INSERT INTO Products (Id,Name,Description,Price,ImageName)" +
                         $" VALUES ({prod.Id},'{prod.Name}','{prod.Description}',{prod.Price.ToString(new CultureInfo("en-US"))}," +
                         $"'{prod.ImageName}');";
                    var numRec = await db.CommandAsync(query);
                }
                catch (Exception ex)
                {
